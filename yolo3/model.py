...
def preprocess_true_boxes(true_boxes, input_shape, anchors, num_classes):
    ...
    for b in range(m):
        ...
        # Discard zero rows.
            ...
            for t, nn in enumerate(best_anchor):
                for ll in range(3):
                    if nn in anchor_mask[ll]:
    #                    try:
                        i = np.floor(true_boxes[b,t,0]*grid_shapes[ll][1]).astype('int32')
                        j = np.floor(true_boxes[b,t,1]*grid_shapes[ll][0]).astype('int32')
                        if j > 51:
                            j = 51
                        nn = anchor_mask[ll].index(nn)
                        c = true_boxes[b, t, 4].astype('int32')
                        y_true[ll][b, j, i, nn, 0:4] = true_boxes[b,t, 0:4]
                        y_true[ll][b, j, i, nn, 4] = 1
                        y_true[ll][b, j, i, nn, 5+c] = 1
    #                    except:
    #                        import pdb
    #                        pdb.set_trace()
                        break
